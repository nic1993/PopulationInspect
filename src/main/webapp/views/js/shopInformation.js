function getShopInformation(policeId,commId,buildId,roomClass){
    var dataInforamtion = {};
    if(policeId != "" && policeId != undefined && policeId != null){
     ...
     if(buildId != "" && buildId != undefined && buildId != null){
		...
		var informationCaption = $('#informationShopCaption');
		var informationTittle = $('#informationshopTittle');
	        ...
	        var informationThead = $('#informationShopThead');
			...
			'<th style="display:none">人员备注</th>'+
			'</tr>';
	        informationThead.html(thead);
	        var informationTbody = $('#informationShopTbody');
	         ...
	         var body = "";
				for(var i=0;i<dataResult.length;i++){
					...
					if(dataResult[i].persFocus == undefined || dataResult[i].persFocus == "" || 
					...
					if(dataResult[i].persRamark == undefined || dataResult[i].persRamark == "" || 
		                ...
		                body += '<td style="display:none">'+dataResult[i].persRamark+'</td>';
					}
	          ...
	          //excel下载按钮
			var button = $("#informationShopButton");
			    button.html("");
			    var buttonC = '';
			if(body != ""){
                buttonC += '<button class="btn btn-info" type="button" onclick="confirmShopInforExcel()">'+
                '导出详情到excel表</button>';
			}
...
function confirmShopInforExcel(){
...
function shopexcel(){
	var title = $("#informationshopTittle").text();
      $("#informationShoptable").table2excel({
                  exclude: ".noExl",
